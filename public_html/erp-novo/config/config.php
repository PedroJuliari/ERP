<?php


define("SERVIDOR", "localhost");
define("BANCO", "erp");
define("USUARIO", "root");
define("SENHA", "");


define('CONTROLLER_PADRAO', 'home');
define('METODO_PADRAO', 'index');
define('NAMESPACE_CONTROLLER', 'app\\controllers\\');
define('URL_BASE', 'http://localhost/erp-novo/');
define('URL_IMAGEM', 'http://localhost/erp-novo/assets/img/');

/*
$config_upload["verifica_extensao"] = false;
$config_upload["extensoes"] = array(".gif",".jpeg", ".png", ".bmp", ".jpg");
$config_upload["verifica_tamanho"] = true;
$config_upload["tamanho"] = 2097152; //2 mb
$config_upload["caminho_absoluto"]= "C:/xampp/htdocs/erp_final/layout_erp/upload/";
$config_upload["renomeia"] = true;
*/
