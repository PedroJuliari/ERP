    ...
    <div class="conteudo-fluido">	
        <div class="rows">	
            ...
            <span class="h5 caixa-titulo"><i class="fas fa-search"></i> Buscar por Pedidos</span>
                     <div class="px-5">
                    <form name="busca" action="<?php echo URL_BASE ."pedido/filtro" ?>" method="GET" >
                    <div class="rows pt-3 pb-4">
                            <div class="col-3">
                                 ...
                                 <div class="col-4">
                                         ...
                                         <select class="form-campo" name="idStatus">
											...
											<?php foreach($status as $s){
												$selecionado = (!isset($pesquisa)) ? "" : ($s->id_status_pedido==$pesquisa->idStatus) ? "selected" : "";
												echo "<option value='$s->id_status_pedido' $selecionado>$s->status_pedido</option>";
											}
											 ?>
                                         </select>
                                 </div>
								 <div class="col-4">
										 ...
										 <input type="text" name="cliente" value="<?php echo isset($pesquisa) ? $pesquisa->cliente: "" ?>" placeholder="Digite aqui..." class="form-campo">
        ...
        <tr>
                ...
                <td align="center"><span class="status <?php echo $classe_status ?>"><?php echo $pedido->status_pedido ?></span></td> 											
			   ...
			   <td align="center">
                    <?php if($pedido->id_status== 1){ ?>                         
                        <a href="javascript:;" class="d-block btn btn-outline-verde status digitacao"> Em Digitação</a>
                    <?php }else if($pedido->id_status==2){ ?>   
                       <a href="<?php echo  URL_BASE ."pedido/verificar_pedido/".$pedido->id_pedido ?>" class="d-block btn btn-outline-verde status emespera"> Atender </a>	
                    <?php }else if($pedido->id_status==3){ ?>
                       <a href="#" class="d-block btn btn-outline-verde status atendido"> Aguardando Produção</a>	
					<?php }else if($pedido->id_status==4){ ?>
                       <a href="<?php echo URL_BASE ."pedido/faturar/" . $pedido->id_pedido ?>" class="d-block btn btn-outline-verde status atendido"> Faturar</a>
                    <?php  } else { ?>
                       <a href="<?php echo URL_BASE ."notafiscal/create/" . $pedido->id_pedido ?>" class="d-block btn btn-outline-verde status atendido"> Emitir NFE</a>	
                     <?php } ?>
                </td>
        </tr>
            <?php } } ?>
